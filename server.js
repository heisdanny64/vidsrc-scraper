...
// vidsrc.xyz and vidsrc.net removed — ERR_NAME_NOT_RESOLVED on Render's infrastructure
const PROVIDERS = [
  "https://vidsrc.in",
  "https://vidsrc.pm",
];
...
async function scrapeProvider(domain, url) {
  ...
  try {
    ...
    // Try to find and click #the_frame to trigger stream load.
    // Use .catch(() => null) so a timeout does NOT throw — some providers
    // (e.g. vidsrc.pm) fire the HLS request passively via the route interceptor
    // before the click, so a missing #the_frame shouldn't be fatal.
    const frameDiv = await page
      .waitForSelector("#the_frame", { timeout: 10000 })
      .catch(() => null);
    ...
    if (frameDiv) {
      ...
      // #the_frame not found — check if HLS was already captured passively
      console.warn(
